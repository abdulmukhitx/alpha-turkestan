"""
GeoAI-TKO · src/processing/verify_overview_2024.py
=======================================================
ЭТАП 4 (2024_summer) — независимая верификация overview-пирамиды
(Урок 6, полный цикл). Тот же скрипт, что verify_overview_2023_cdse.py,
только пути под 2024.

Три проверки:
  1. Overview levels присутствуют и корректны (метаданные)
  2. 6 точек по всей площади (full extent), выровненных на границу блока
     64x64 — full-res блок и overview-пиксель (factor=64) должны
     показывать одинаковый nodata-статус и близкие значения
  3. Для 3 тайлов с n_unique_orbits_in_top3=1 (41TQH, 42TXK, 42TXL —
     те же самые тайлы, что и в 2023/2025, структурная особенность
     геометрии орбит) — реальный full-res nodata% ВНУТРИ официальной
     границы (Урок 10) в финальном смердженном файле

Usage:
  python src/processing/verify_overview_2024.py
"""
import json
import sys
...
REPROJ_DIR = Path(r"D:\data\s2_2024_reproj")
...
def main():
    ...
    out_path = Path(r"D:\data\mosaics\2024_summer\overview_verification.json")
