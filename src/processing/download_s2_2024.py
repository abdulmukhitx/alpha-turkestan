"""
GeoAI-TKO · src/processing/download_s2_2024.py
=======================================================
ЭТАП 2 (2024_summer) — скачивание top-3 orbit-diverse продуктов
(primary/fill1/fill2) на каждый из 43 master tile, СРАЗУ как единый набор
(тот же пайплайн, что 2023_summer_cdse) — план уже отобран в Этапе 1
(composite_candidates.json).

Качает только нужные JP2-ассеты (B02,B03,B04,B05,B08,B8A,B11) в
D:\\data\\s2_2024_raw\\{tile}\\{primary|fill1|fill2}\\{band}.jp2.
...
build_mosaic_2025.baseline_offset — тот же метод, что и 2023/2025, не
переизобретается), n_unique_orbits_in_top3.

Запуск (обычно через detached PowerShell Start-Process, см. Этап 2 план):
  python src/processing/download_s2_2024.py > D:\\data\\s2_2024_raw\\download.log 2>&1
"""
import json
import sys
import time
...
CANDIDATES_PATH = Path(r"D:\data\s2_2024_raw\composite_candidates.json")
RAW_DIR = Path(r"D:\data\s2_2024_raw")
...
# DecodeError on CDSE's brotli-compressed responses.
...
def main():
    t0 = time.time()
    print("=" * 70)
    print("  GeoAI-TKO: Этап 2 — скачивание Sentinel-2 L2A, лето 2024 (CDSE, top-3 orbit-diverse)")
    print("=" * 70)
    ...
    manifest = {"year": 2024, "source": "cdse_direct", "generated_at": None, "tiles": {}}
